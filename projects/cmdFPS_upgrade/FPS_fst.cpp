/*2025.9.30-2025.10.4 Done with this cpp object, followed with tutorial of javidx9 from Youtube*/
#include <iostream>
#include <vector>
#include <utility>
#include <algorithm>
...
public:
	...
	virtual bool OnUserCreate() {
		...
		spriteWall = new olcSprite(L"includes/fps_wall1.spr");
		spriteLamp = new olcSprite(L"includes/fps_lamp1.spr");
		spriteFireBall = new olcSprite(L"includes/fps_fireball1.spr");
	...
	virtual bool OnUserUpdate(float fElapsedTime) {
		...
		if (m_keys[L'E'].bHeld) {
			...
			if (map.c_str()[(int)fPlayerY * nMapWidth + (int)fPlayerX] == L'#') {
				fPlayerX -= cosf(fPlayerA) * fSpeed * fElapsedTime;
				fPlayerY += sinf(fPlayerA) * fSpeed * fElapsedTime;
			}
		}
			...
			while (!bHitWall && fDistanceToWall < fDepth)
			{
				...
				else
				{
					...
					if (map.c_str()[nTestY * nMapWidth + nTestX] == '#')
					{
						...
						float fTestPointX = (float)fPlayerX + fEyeX * fDistanceToWall;
						float fTestPointY = (float)fPlayerY + fEyeY * fDistanceToWall;
						...
						if (fTestAngle >= 3.14159f * 0.25f && fTestAngle < 3.14159 * 0.75f) {
							fSampleX = fTestPointX - (float)nTestX;
						}
						if (fTestAngle >= 3.14159f * 0.75f || fTestAngle < -3.14159f * 0.75f) {
							fSampleX = fTestPointY - (float)nTestY;
						}
						if (fTestAngle >= -3.14159f * 0.75f && fTestAngle < -3.14159f * 0.25f) {
							fSampleX = fTestPointX - (float)nTestX;
						}
		...
		for (auto& object : listObjects) {
			...
			// Check if object is inside the wall--set flag for removal
			...
			float fDistanceFromPlayer = sqrt(pow(fVecX, 2) + pow(fVecY, 2));
			...
			if (bInPlayerFOV && fDistanceFromPlayer < fDepth && fDistanceFromPlayer >= 0.5f) {
		...
		Draw(fPlayerX + 1, fPlayerY + 1, L'P');
